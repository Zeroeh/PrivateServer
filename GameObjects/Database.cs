                    ...
                    while (rdr.Read())
                        ret.Add(new NewsItem()
                        {
                            ...
                            Title = string.Format("Your {0} died at level {1}",
                                ...
                                rdr.GetInt32("level")),
                            TagLine = string.Format("You earned {0} Fame",
        ...
        public Char LoadCharacter(Account acc, int charId)
        {
            ...
            using (var rdr = cmd.ExecuteReader())
            {
                ...
                var ret = new Char()
                {
                    ...
                    Pet = rdr.GetInt32("pet"), //petcode
                    PCStats = rdr.GetString("fameStats")
                };
                ret.FameStats = new FameStats();
                if (!string.IsNullOrEmpty(ret.PCStats))
                    ret.FameStats.Read(ZlibStream.UncompressBuffer(Convert.FromBase64String(ret.PCStats.Replace('-', '+').Replace('_', '/')))); //64bit characters something blah blah debugger errors
                return ret;
            }
        }
...
WHERE accId=@accId AND charId=@charId;";
            ...
            cmd.Parameters.AddWithValue("@pet", chr.Pet); //petcode IS NEEDED!!!
...
VALUES(@accId, @chrId, @name, @objType, @tex1, @tex2, @items, @fame, @fameStats, @totalFame, @firstBorn, @killer);";
        ...
        public List<int> getLockeds(int accId) //marked for deletion
        {
            var cmd = CreateQuery();
        ...
        public List<int> getIgnoreds(int accId) //marked for deletion
        {
            var cmd = CreateQuery();
            ...
            try
            {
                ...
                foreach (var i in cmd.ExecuteScalar().ToString().Split(','))
                    ignores.Add(int.Parse(i)); //This line is messed up. No definition for "i"
                return ignores;
            }
            catch
            { return new List<int>(); }
        }
        public bool AddLock(int accId, int lockId) //marked for deletion
        {
        ...
        public bool RemoveLock(int accId, int lockId) //marked for deletion
        {
        ...
        public bool AddIgnore(int accId, int ignoreId) //marked for deletion
        {
        ...
        public bool RemoveIgnore(int accId, int ignoreId) //marked for deletion
        {
