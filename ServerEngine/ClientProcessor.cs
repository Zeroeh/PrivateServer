...
{
    public enum ProtocalStage //the common 4-way handshake network method
    {
        Connected,
        Handshaked,
        Ready,
    ...
    public class ClientProcessor //Structure for NetworkTicker to accept the handshake, then transmit the encrypted packets
    {
        Socket skt;
        Thread wkrThread;
        ...
        public string clientVer = "1.0.0"; //Need to change in client
        ...
        //internal bool ProcessPacket(Packet pkt); //was commented
        public bool IsReady()
        {
        ...
        internal void ProcessPacket(Packet pkt) //Our packets are processed in this order...
        {
            try
            {
                if (pkt.ID == PacketID.Hello) //Hello packet must get priority
                    ProcessHelloPacket(pkt as HelloPacket);
                else if (pkt.ID == PacketID.Create) //create any data that the server must have
                    ProcessCreatePacket(pkt as CreatePacket);
                else if (pkt.ID == PacketID.Load) //load any data the server sends back
                    ProcessLoadPacket(pkt as LoadPacket);
                else if (pkt.ID == PacketID.Pong) //pong back to the server saying that you're fully connected
                    entity.Pong(pkt as PongPacket);
                else if (pkt.ID == PacketID.Move) //we can now go about our normal packet sending
        ...
        public void Disconnect()
        {
            try
            {
                ...
                skt.Close(); //client is no longer connected, tell the server to stop transmitting data and drop the connection
            }
            catch
            {
        ...
        public void Save() //Saves our character data
        {
            try
            {
                if (db != null)
                {
                    if (character != null) //if our character doesn't exist, or is in the map editor...
                    {
                        entity.SaveToCharacter();
                        if (entity.Owner.Id != -6)
                            db.SaveCharacter(account, character);
                    }
                    db.Dispose(); //... we don't save
                    db = null;
                }
                else
                {
        ...
        void ProcessCreatePacket(CreatePacket pkt)
        {
            ...
            cmd.CommandText = "INSERT INTO characters (accId, charId, charType, level, exp, fame, items, hp, mp, stats, dead, pet, fameStats) VALUES (@accId, @charId, @charType, 1, 0, 0, @items, 100, 100, @stats, FALSE, -1, @fameStats);"; //pets was right after dead
        ...
        public bool CheckAccountInUse(int accId)
        {
            ...
            {
                Console.WriteLine("Error checking if account with ID: " + accId + " is in use, check ClientProcessor.cs");
                return false;
            }
        }
        //Possible anti-cheat measure
        ...
        {
        //    this.UpdateAckCount++;
        //    if ((this.Player.UpdateCount - (int)this.UpdateAckCount) > 1000)
        //    {
        //        Console.WriteLine("Possible custom client (UpdateAck count lesser than UpdateCount): " + this.Account.Name);
        //    }
        //    if ((this.Player.UpdateCount - (int)this.UpdateAckCount) > 10000)
        //    {
        //        Console.WriteLine("Provable custom client (UpdateAck count lesser than UpdateCount): " + this.Account.Name + ", disconnecting it");
        //        psr.SendPacket(new svrPackets.FailurePacket() { Message = "Provable hacked client detected, disconnecting..." });
        //        psr.Disconnect();
        //    }
        }
    }
