        ...
        public override void Buy(Player player)
        {
            if (ObjectType == 0x01ca)   //Merchant
            {
                if (TryDeduct(player))
                {
                    ...
                    Console.WriteLine("[{3}]User {0} has bought {1} for {2}", player.nName, ItemName, truePrice, DateTime.Now);
                    using (System.IO.StreamWriter writer = new System.IO.StreamWriter(@"logs\PurchaseLog.txt", true))
                    {
                        writer.WriteLine("[{3}]User {0} has bought {1} for {2}", player.nName, ItemName, truePrice, DateTime.Now);
                    } //end of logging code
                }
                else
