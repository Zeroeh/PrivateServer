...
namespace ServerEngine.realm.commands
{
		...
		public void Execute(Player player, string[] args)
		{
			...
			else
			{
				foreach (var i in player.Owner.EnemiesCollision.HitTest(player.X, player.Y, 8).OfType<Enemy>())
				{
					...
					{
						player.SendInfo("Monsters in the area block your effort to pause.");
						return;
					}
				}
				player.ApplyConditionEffect(new ConditionEffect()
	...
	class TellCommand : ICommand
	{
		...
		public void Execute(Player player, string[] args)
		{
			try
			{
				...
				string[] tags = {/*"[P]", "[Advisor]", "[Donator]", "[GM]", "[GM+]", "[GM++]", "[Overseer]", "[Lord]"*/};
				...
				{
					player.SendInfo(string.Format("Why are you talking to yourself?"));
					return;
				}
	...
	class SeeCommand : ICommand
	{
		public string Command { get { return "see"; } }
		public int RequiredRank { get { return 0; } }
		...
		public void Execute(Player player, string[] args)
		{
			...
			try
			{
				...
				player.SendHelp("Use /see <playername>");
			}
			catch
			{
	...
	{
		public string Command { get { return "allcommands"; } }
		public int RequiredRank { get { return 1; } }
	...
	class ForgeListCommand : ICommand
	   ...
	   class AdminChat : ICommand //This still needs to be tested
	   {
		   public string Command { get { return "@"; } }
		   public int RequiredRank { get { return 0; } }
		   ...
		   public void Execute(Player player, string[] args)
		   {
			   ...
			   if (player.Client.Account.Rank >= 0)
			   {
				   ...
				   try
				   {
					   ...
					   foreach (var w in RealmManager.Worlds)
					   {
						   ...
						   if (w.Key != 0)
						   {
							   foreach (var i in world.Players)
							   {
								   if (i.Value.Client.Account.Rank >= 1)
								   {
				   ...
				   {
					   player.SendInfo("Cannot admin chat!");
				   }
			   }
			   else
				   player.SendInfo("You need to be an admin to use this command.");
		   }
	   }
	   class PremChat : ICommand
	   {
		   public string Command { get { return "t"; } } //Chat used to buy and sell items. Spammers will get fucked.
		   public int RequiredRank { get { return 0; } }
		   ...
		   public void Execute(Player player, string[] args)
		   {
			   if (args.Length == 0)
			   {
				   ...
				   if (player.Client.Account.Rank >= 0)
				   {
					   ...
					   {
						   player.SendInfo("Cannot trade chat!");
					   }
				   }
				   else
					   player.SendInfo("You need to be a player to use this command.");
		   }
	   }
	   class PetMode : ICommand
	   {
		   public string Command { get { return "petmode"; } }
		   public int RequiredRank { get { return 0; } }

		   public void Execute(Player player, string[] args)
		   {
			   if (player.HasConditionEffect(ConditionEffects.Healing))
			   {
				   player.ApplyConditionEffect(new ConditionEffect()
				   {
			   ...
			   else
			   {
				   ...
				   foreach (var i in RealmManager.Clients.Values)
					   i.SendPacket(new NotificationPacket()
					   {
						   Color = new ARGB(0xff0000),
						   ObjectId = player.Id,
						   Text = "Petmode Activated"
					   });
			   }
		   }
	   }
	   class Godland : ICommand //command is in beta, doesn't work atm
	   {
		   ...
		   public void Execute(Player player, string[] args)
		   {
			   if (player.Owner is GameWorld)
			   {
				   ...
				   player.SendInfo("Map is not 1000x1000.");
		   }
	   }
	   //class FlowCommand : ICommand
	   //{
	   //	public string Command { get { return "flow"; } }
	   //	public int RequiredRank { get { return 0; } }
	   ...
	   //		{
	   //			foreach (var i in RealmManager.Clients.Values)
	   //				i.SendPacket(new ShowEffectPacket()
	   //				{
	   //					EffectType = EffectType.Trail,
	   //					TargetId = player.Id,
	   //					PosA = new Position() { X = player.X, Y = player.Y },
	   //					//Duration = 0, //Experimental
	   //					Color = new ARGB(0x000500)
	   //				});
	   //		}
	   //		catch
	   //		{
	   //		 player.SendHelp("This message should not be showing. Please report to an admin.");
	   //		}
	   //	}
	   //}
	   class FlowMode : ICommand
	   {
		   public string Command { get { return "trail"; } }
		   public int RequiredRank { get { return 0; } }

		   public void Execute(Player player, string[] args)
		   {
			  // for (int u = 0; u < 2147683; u++)
			   //{
				   ...
				   foreach (var i in RealmManager.Clients.Values)
					   i.SendPacket(new ShowEffectPacket()
					   {
						   ...
						   TargetId = player.Id,
						   PosA = new Position() {X = player.X, Y = player.Y},
						   //Duration = -1, //Experimental
						   Color = new ARGB(0x000500)
					   });
				   player.SendInfo("Trails Enabled");
			   //}
		   }
	   }
}
